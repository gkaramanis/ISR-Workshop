...
![Artwork by Allison Horst](images/ggplot-ah.png){width="50%"}
...
![](images/ggplot-layers.png){width="65%"}
...
For the exercises we will use two datasets, [palmerpenguins](https://allisonhorst.github.io/palmerpenguins/) and [friends_info](https://github.com/rfordatascience/tidytuesday/blob/master/data/2020/2020-09-08/readme.md). Don't worry about installing or downloading anything, the datasets are already loaded for you. The links are for more information about the data.
...
2.  For the x-axis, look for a date column.
...
```{webr}
#| warning: false
ggplot(palmerpenguins, aes(x = species, y = body_mass_g)) +
  geom_boxplot()
```
...
::: solution
``` r
ggplot(friends_info, aes(x = season, y = us_views_millions)) +
  geom_boxplot()
```
:::
::::::
...
While Guideline 5 focuses on time series, the same principle applies to any variables with large ranges. Let's look at ggplot2's [mammals sleep dataset](https://ggplot2.tidyverse.org/reference/msleep.html).\ 
...
1. Run the code\
2. Remove one of the lines with `scale_*_log10()` and rerun the code
...
![](images/pitfall_verticaltext.jpg)
